...
/**
 * protection: A geoprocessing function that calculates overlap metrics
...
export async function protection(
  ...
  const sketchCategoryMap = getIucnCategoryNameForSketches(
  ...
  const sketchLevelMap = getIucnLevelNameForSketches(toSketchArray(sketch));
  ...
  return {
    metrics: sortMetrics(
      rekeyMetrics([...metrics, ...categoryMetrics, ...levelMetrics]),
    ),
...
export default new GeoprocessingHandler(protection, {
  title: "protection",
  description: "protection level overlap",
  timeout: 500, // seconds
